resource "aws_cloudfront_distribution" "roboshop" {
  ...
  # Cache behavior with precedence 0 (aws check once)
  ...
  restrictions {
    geo_restriction {
      ...
      locations        = ["US", "IN", "TH", "DE"]  #countries we need to use cloufront
    }
  }
...
resource "aws_route53_record" "cdn" {
  zone_id = var.zone_id
  name    = "${var.environment}.${var.domain_name}" # dev.ashok.sbs
  type    = "A"
